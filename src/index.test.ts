...
import * as bsearch from "./index.ts";
...
describe("largestFloatAsync", () => {
  it("finds the largest floating point number", async () => {
    ...
    const idealFontSize = (text: string) => {
      return bsearch.largestFloatAsync(1, 64, async (size) => measureText(size, text) <= 1280);
    };
    expect(await idealFontSize("hi")).toBe(64);
    expect(await idealFontSize("there is just too much text to fit here!")).toBe(32);
    expect(
      await idealFontSize("there is just too much text to fit here so it has to be scaled down"),
